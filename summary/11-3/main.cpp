...
public:
	...
	int findnearparented(int i,int j)
	{
	    if(i==j) return i;
	    if(i>j) {i=i/2; return findnearparented(i,j);}
	    else {j=j/2; return findnearparented(i,j);}
	}
	Type findnearparent(Type x,Type y)
	{
	    int a,b,i,j;
	    for(i=1;i<maxsize;++i)   //查找x在数组data中的下标
	         if(data[i]==x) break;
        if(i<=maxsize) a=i;
        else {cout<<"二叉树上没有元素"<<x<<endl;return x;}
        for(j=1;j<maxsize;++j)       //查找y在数组data中的下标
	         if(data[j]==y) break;
        if(j<=maxsize) b=j;
        else {cout<<"二叉树上没有元素"<<y<<endl;return y;}
        return data[findnearparented(a,b)];
    }
};
...
int main()
{
	...
	tree.createTree('@');
	cout<<"请输入查找哪两个元素的最近的共同祖先：";
	cin>>k>>l;
	cout<<k<<"和"<<l<<"的最近共同祖先是"<<tree.findnearparent(k,l);
    return 0;
}
